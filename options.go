...
const (
	...
	// any activity it pings the gmserver to see if the transport is still alive.
	...
	// MaxSendMsgSize set max gRPC request message size sent to gmserver.
	// If any request message size is larger than current value, an error will be reported from gRPC.
	MaxSendMsgSize = 4 << 20

	// MaxRecvMsgSize set max gRPC receive message size received from gmserver.
